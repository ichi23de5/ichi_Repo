...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    @api.multi
    def _create_invoice(self, order, so_line, pp_line, amount):
        pass
...
## add code from here
#            'property_line_ids': [(6, 0, [pp_line.id])],
            ...
            #        'completion_date': order.completion_date,
             #       'price_total': order.amount_untaxed,
              #      'purchase_number': order.purchase_number or False,
               #     'type_id': order.act_type,
               # })
 #               self._create_pp_line()
#                self._create_invoice(order, so_line, pp_line, amount)
...
#    def _create_pp_line(self):
#        so_set = self.env['sale.order']
#        p_obj = self.env['account.property.line']
#        val = {}
#        for so in so_set:
#            val = { 
#                'invoice_id':so.id,
#                'origin': so.name,
#                'property_id': so.property_id.id or False,
#                'completion_date': so.completion_date,
#                'price_total': so.amount_untaxed,
#                'purchase_number': so.purchase_number or False,
#                'type_id': so.act_type,
#            }   
#            p_obj.create(val)
